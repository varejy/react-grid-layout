...
export function getLayoutItem(layout: Layout, id: string): ?LayoutItem {
  for (let i = 0, len = layout.length; i < len; i++) {
    let layoutId = layout[i].i
    
    if (layoutId === id || Array.isArray(layoutId) && layoutId.includes(id)) return layout[i];
  }
}

export function getChildren(children: ReactNode, layoutItem: LayoutItem): ?ReactNode[] {
  return children
    .filter(
      (child) =>
        child.key &&
        (
          layoutItem.i == child.key ||
          Array.isArray(layoutItem.i) && layoutItem.i.includes(child.key)
        )
    )
}
...
export function synchronizeLayoutWithChildren(
  ...
  React.Children.forEach(children, (child: ReactElement<any>) => {
    ...
    if (exists) {
      if (!layout.find((item) => isEqual(item, exists)))
...
export function memoize(callback, resolver, cacheType = WeakMap) {
